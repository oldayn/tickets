...
<head>
  <meta charset="UTF-8" />
  <link rel="icon" type="image/png" href="/z/fav.png" sizes="512x512">
  <style>
...
$sql = "select (sp_date || ' ' || hall_sname || ' ' || sp_name) as sp, count(*), ticket.price, s_d, ts_time, sum(ticket.price) from w_get_spec1 left join ticket on w_get_spec1.idspec = ticket.id_spec where ticket.id_orders = '$order' group by sp, price, s_d, ts_time order by s_d, ts_time;";
$res = pg_query($sql);
